...
vs_real vs_seconds_elapsed(void)
{
  ...
  SYSTEMTIME time;
  GetSystemTime(&time);
  return (vs_real)time.wDay * 24 * 3600.0 + time.wHour * 3600.0 + time.wMinute * 60.0 + time.wSecond + time.wMilliseconds * 0.001;
#else
  time_t now;
  struct tm *date;
...
vs_real vs_step_smooth(vs_real x, vs_real x0, vs_real h0, vs_real x1, vs_real h1)
{
  if (x <= x0)
    ...
    return h0 + 0.5 * (h1 - h0) * (1.0 - cos(PI * (x - x0) / (x1 - x0)));
}
                             ...
                             unsigned int size)
{
  ...
  if (expand)
  {
    ...
    if (strlen(string) + count * (lenr - lenf) > size - 1)
      return NULL;
  }
